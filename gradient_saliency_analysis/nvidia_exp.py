...
def load_model(model_name):
    ...
    model = AutoModel.from_pretrained(model_name, trust_remote_code=True, torch_dtype="float16").to(device)
    model.eval() 
...
def get_saliency_map(model, tokenizer, query, document, pooling_method="last"):
    with torch.no_grad():
        q_inputs = tokenizer(query + tokenizer.eos_token , return_tensors="pt", padding=True, truncation=True, max_length=512).to(device)
        q_outputs = model(**q_inputs)
        
        if pooling_method == "mean":
            q_emb = mean_pooling(q_outputs["sentence_embeddings"], q_inputs['attention_mask'])
        else: # last
            q_emb = last_token_pooling(q_outputs.last_hidden_state, q_inputs['attention_mask'])
        
        q_emb = F.normalize(q_emb, p=2, dim=1)

    d_inputs = tokenizer(document + tokenizer.eos_token, return_tensors="pt", truncation=True, max_length=2048).to(device)
    input_ids = d_inputs['input_ids']
    attention_mask = d_inputs['attention_mask']

    embedding_layer = model.embedding_model.embed_tokens
    # print(embedding_layer)
    inputs_embeds = embedding_layer(input_ids).detach()
    inputs_embeds.requires_grad = True 
    inputs_embeds.retain_grad()

    d_outputs = model(input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask)
    
    if pooling_method == "mean":
        d_raw_emb = mean_pooling(d_outputs["sentence_embeddings"], attention_mask)
    else: # last
    ...
    saliency_scores = torch.norm(gradients[1:-1], dim=1).cpu().numpy() # remove special tokens
...
for domain in tqdm.tqdm(os.listdir(language_dir)):
    ...
    json_file=f"evaluation_results/NV-Embed-v2/eng-eng/{domain}.json"
